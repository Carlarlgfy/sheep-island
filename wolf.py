...
WOLF_HUNT_RADIUS          = 52.0   # tile radius for hunting living sheep
WOLF_SMELL_RADIUS         = 75.0   # tile radius for smelling fresh corpses
...
WOLF_SMELL_MEAT_THRESHOLD = 0.5    # only sniff out corpses if hunger above this
...
class Wolf:
    ...
    def _find_best_prey(self, sheep_list: list):
        """Return the best living-sheep target within hunt radius, or None."""
        hunt_r_sq    = WOLF_HUNT_RADIUS ** 2
        best_score   = -1.0
        best_target  = None
    ...
    def _find_nearest_corpse(self, sheep_list: list):
        ...
        for sheep in sheep_list:
            ...
            ddy     = sheep.ty - self.ty
            dist_sq = ddx * ddx + ddy * ddy
            if dist_sq < smell_sq and dist_sq < best_dist_sq:
                best_dist_sq = dist_sq
                best_corpse  = sheep
    ...
    def _do_lunge_damage(self, target) -> bool:
        ...
        dmg = self.genetic_strength * random.uniform(*WOLF_DAMAGE_BASE)
    ...
    def _move_toward(self, tx: float, ty: float, dt: float,
        ...
        """Move toward (tx, ty), apply separation, clamp to non-water."""
        ddx  = tx - self.tx
        ddy  = ty - self.ty
        dist = math.sqrt(ddx * ddx + ddy * ddy)
        if dist > 0.05:
        ...
        new_tx = self.tx + self.dx * spd * dt + sx
        new_ty = self.ty + self.dy * spd * dt + sy
    ...
    def update(self, dt: float, grid: list, sheep_list: list,
        ...
        if self.state == Wolf.LUNGE:
            ...
            if self._lunge_timer <= 0:
                ...
                if killed:
                    # Move to eat the corpse immediately
                    self.state = Wolf.EAT
        ...
        if self.state == Wolf.HUNT:
            ...
            # If target is now a fresh corpse — switch to eating it
            ...
            if self._scan_timer <= 0:
                ...
                # Check corpses if hungry enough
                corpse = self._find_nearest_corpse(sheep_list)
                if corpse is not None and self.hunger > WOLF_SMELL_MEAT_THRESHOLD:
                    self._hunt_target = corpse
                    self.state        = Wolf.HUNT
                    return
                # Prefer pack's shared target if it's better
        ...
        # Check corpses even when not starving
        if self.hunger >= WOLF_SMELL_MEAT_THRESHOLD and self._scan_timer <= 0:
            corpse = self._find_nearest_corpse(sheep_list)
